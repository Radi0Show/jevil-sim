#!/usr/bin/env node
// Attack 3 acceptance: dc.type 49 (jattack 2 — heart suit bombs) vs the real
...
const oraclePath = join(homedir(), 'jevil-research', 'traces', 'a49-suitbomb.csv');
if (!existsSync(oraclePath)) {
  console.log(`MISSING ${oraclePath} — record with oracle_attack.csx type=49`);
  process.exit(1);
}

const oracleLines = readFileSync(oraclePath, 'utf8').replace(/\r/g, '').replace(/\n$/, '').split('\n');
const { csv, counters } = runTraceFull({ seed: 1, frames: 600, scene: 'oracle-a49' });
