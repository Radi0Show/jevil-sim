#!/usr/bin/env node
// Attack 11 acceptance: dc.type 72 (jattack 10 — Three-Club Attack) vs the real
...
const oraclePath = join(homedir(), 'jevil-research', 'traces', 'a72-clubs.csv');
if (!existsSync(oraclePath)) {
  console.log(`MISSING ${oraclePath} — record with oracle_attack.csx type=72`);
  process.exit(1);
}

const oracleLines = readFileSync(oraclePath, 'utf8').replace(/\r/g, '').replace(/\n$/, '').split('\n');
const { csv, counters } = runTraceFull({ seed: 1, frames: 600, scene: 'oracle-a72' });
