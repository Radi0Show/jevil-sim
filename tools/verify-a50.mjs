#!/usr/bin/env node
// Attack 6 acceptance: dc.type 50 (jattack 5 — club suit bombs) vs the real
...
const oraclePath = join(homedir(), 'jevil-research', 'traces', 'a50-clubbomb.csv');
if (!existsSync(oraclePath)) {
  console.log(`MISSING ${oraclePath} — record with oracle_attack.csx type=50`);
  process.exit(1);
}

const oracleLines = readFileSync(oraclePath, 'utf8').replace(/\r/g, '').replace(/\n$/, '').split('\n');
const { csv, counters } = runTraceFull({ seed: 1, frames: 600, scene: 'oracle-a50' });
