#!/usr/bin/env node
// Attack 10 acceptance: dc.type 48 (jattack 9 — spade suit bombs) vs the real
...
const oraclePath = join(homedir(), 'jevil-research', 'traces', 'a48-spadebomb.csv');
if (!existsSync(oraclePath)) {
  console.log(`MISSING ${oraclePath} — record with oracle_attack.csx type=48`);
  process.exit(1);
}

const oracleLines = readFileSync(oraclePath, 'utf8').replace(/\r/g, '').replace(/\n$/, '').split('\n');
const { csv, counters } = runTraceFull({ seed: 1, frames: 600, scene: 'oracle-a48' });
