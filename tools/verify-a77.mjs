#!/usr/bin/env node
// Attack 16 acceptance: dc.type 77 (jattack 15 — FINAL CHAOS) vs the real
...
const oraclePath = join(homedir(), 'jevil-research', 'traces', 'a77-finalchaos.csv');
if (!existsSync(oraclePath)) {
  console.log(`MISSING ${oraclePath} — record with oracle_attack.csx type=77`);
  process.exit(1);
}

const oracleLines = readFileSync(oraclePath, 'utf8').replace(/\r/g, '').replace(/\n$/, '').split('\n');
const { csv, counters } = runTraceFull({ seed: 1, frames: 600, scene: 'oracle-a77' });
