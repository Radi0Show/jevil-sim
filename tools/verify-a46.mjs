#!/usr/bin/env node
// Attack 14 acceptance: dc.type 46 (jattack 13 — random suit bombs) vs the real
...
const oraclePath = join(homedir(), 'jevil-research', 'traces', 'a46-randombomb.csv');
if (!existsSync(oraclePath)) {
  console.log(`MISSING ${oraclePath} — record with oracle_attack.csx type=46`);
  process.exit(1);
}

const oracleLines = readFileSync(oraclePath, 'utf8').replace(/\r/g, '').replace(/\n$/, '').split('\n');
const { csv, counters } = runTraceFull({ seed: 1, frames: 600, scene: 'oracle-a46' });
