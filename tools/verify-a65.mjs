#!/usr/bin/env node
// Attack 2 acceptance: dc.type 65 (jattack 1 — spade rings) vs the real
...
const oraclePath = join(homedir(), 'jevil-research', 'traces', 'a65-spadering.csv');
if (!existsSync(oraclePath)) {
  console.log(`MISSING ${oraclePath} — record with oracle_attack.csx type=65`);
  process.exit(1);
}

const oracleLines = readFileSync(oraclePath, 'utf8').replace(/\r/g, '').replace(/\n$/, '').split('\n');
const { csv, counters } = runTraceFull({ seed: 1, frames: 600, scene: 'oracle-a65' });
