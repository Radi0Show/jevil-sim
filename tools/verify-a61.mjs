#!/usr/bin/env node
// Attack 9 acceptance vs the real
...
const oraclePath = join(homedir(), 'jevil-research', 'traces', 'a61-carousel2.csv');
if (!existsSync(oraclePath)) {
  console.log(`MISSING ${oraclePath} — record with oracle_attack.csx type=61`);
  process.exit(1);
}

const oracleLines = readFileSync(oraclePath, 'utf8').replace(/\r/g, '').replace(/\n$/, '').split('\n');
const { csv, counters } = runTraceFull({ seed: 1, frames: 600, scene: 'oracle-a61' });
