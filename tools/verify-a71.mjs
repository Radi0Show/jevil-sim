#!/usr/bin/env node
// Attack 13 acceptance: dc.type 71 (jattack 12 — single-diamond teleports) vs the real
...
const oraclePath = join(homedir(), 'jevil-research', 'traces', 'a71-teleport.csv');
if (!existsSync(oraclePath)) {
  console.log(`MISSING ${oraclePath} — record with oracle_attack.csx type=71`);
  process.exit(1);
}

const oracleLines = readFileSync(oraclePath, 'utf8').replace(/\r/g, '').replace(/\n$/, '').split('\n');
const { csv, counters } = runTraceFull({ seed: 1, frames: 600, scene: 'oracle-a71' });
...
if (counters.motionSteps < 300) {
  console.log(`POSITIVE ASSERTION FAILED: motion steps ${counters.motionSteps} < 300`);
  bad = true;
}
