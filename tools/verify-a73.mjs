#!/usr/bin/env node
// Attack 7 acceptance: dc.type 73 (jattack 6 — Diamond Release) vs the real
...
const oraclePath = join(homedir(), 'jevil-research', 'traces', 'a73-diamonds.csv');
if (!existsSync(oraclePath)) {
  console.log(`MISSING ${oraclePath} — record with oracle_attack.csx type=73`);
  process.exit(1);
}

const oracleLines = readFileSync(oraclePath, 'utf8').replace(/\r/g, '').replace(/\n$/, '').split('\n');
const { csv, counters } = runTraceFull({ seed: 1, frames: 600, scene: 'oracle-a73' });
