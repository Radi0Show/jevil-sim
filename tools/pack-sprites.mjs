...
// DIMS CHECK (CLAUDE.md: "ALWAYS verify PNG dims == manifest dims after").
// A trimmed-without-padding export ships a smaller PNG and every draw
// shifts by the trim offset — knight found it via a sword 28px off its
// column. PNG IHDR: width/height big-endian at bytes 16-23.
let dimsBad = 0;
for (const [name, m] of Object.entries(manifest)) {
  for (const f of m.files) {
    ...
    if (w !== m.w || h !== m.h) {
      console.error(`DIMS MISMATCH ${f}: png ${w}x${h} vs manifest ${m.w}x${m.h}`);
      dimsBad += 1;
    }
  }
}
if (dimsBad) {
  console.error(`${dimsBad} frame(s) failed the dims check — re-export WITH padding`);
  process.exit(1);
}
