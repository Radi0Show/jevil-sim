...
// Chapter 1 note: spr_dodgeheartmask and spr_battlebg_0 are BIT-IDENTICAL to
// chapter 3's (compared against knight-sim's extracted data during recon), so
// every calibration below carries over with its evidence intact. The data
// module is still generated from the CHAPTER 1 dump — never copied.
 ...
 */
// (knight-sim's custom-arena spr_battlebg_stretch_hitbox entry is not carried:
 ...
 * `Precise` uses the pixel grid `masksOverlap` walks (spr_dodgeheart itself
 * is AxisAlignedRect; the soul's real mask is the Precise
 * spr_dodgeheartmask). RotatedRect shapes for chapter 1 bullets get their
 * metadata entries here as their attacks are translated.
 */
